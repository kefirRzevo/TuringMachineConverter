...
class MachineExecutor {
  ...
  void initProgramOptions() {
    ...
    config.add_options()
    ("in", po::value<std::string>()->required(), inDesc.c_str())
    ("out", po::value<std::string>()->implicit_value(""), outDesc.c_str())
    ("dump", po::value<DumpLvl>()->implicit_value(1), "Dump Level during execution");
  ...
  void parseProgramOptions(int argc, const char* argv[]) {
    ...
    try {
      ...
      if (vm.count("out")) {
        ...
        auto clear = fullpath.replace_extension().generic_string();
...
public:
  MachineExecutor(std::string_view name)
  : name_(name) {}
...
class MachineConverter {
  ...
  void initProgramOptions() {
    ...
    config.add_options()
    ("in", po::value<std::string>()->required(), inDesc.c_str())
    ("out", po::value<std::string>()->required(), outDesc.c_str());
    cmdline_.add(generic).add(config);
  }
...
public:
  MachineConverter(std::string_view name)
  : name_(name) {}
